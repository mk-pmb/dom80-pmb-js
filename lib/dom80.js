...
(function () {
  ...
  D.setWindow = function setWindow(w) {
    D.win = w;
    D.doc = w.document;
    return D;
  };
  ...
  D.mkTagC = (function prepare() {
    ...
    function mkTagC(spec, opt) {
      ...
      var el = spec[1], par = orf(opt.parent), prevSpecParam = '';
      if (el === ':') {
      ...
      spec.forEach(function applyFx(s) {
        if (!s) { return; }
        var param = s.slice(1) || prevSpecParam;
        fx[s.slice(0, 1)](param, el, opt);
        prevSpecParam = param;
      });
  ...
  D.setAttr = function (e, a, v) {
    ...
    if (D.ifObj(a)) {
      D.mapkv(D.setAttr.bind(null, e), a);
      return e;
    }
    e.setAttribute(a, v);
    return e;
  ...
  D.attrDict = function (e) {
    var a = e.attributes, d = Object.create(null), i, n = a.length, p;
    for (i = 0; i < n; i += 1) {
      p = a[i];
      d[p.nodeName] = p.nodeValue;
    }
    return d;
  };
  ...
  D.mapkv = function (f, o) {
    const r = {};
    Object.keys(o).forEach(function each(k) { r[k] = f(k, o[k], o); });
    return r;
  };
  D.mapvk = function (f, o) {
    const r = {};
    Object.keys(o).forEach(function each(k) { r[k] = f(o[k], k, o); });
    return r;
  };
  ...
  (function () {
    function c(e, k, v) {
      var s = orf(e.style);
      if (!k) { return orf(s); }
      if (v === undefined) { return ores(s[k]); }
      s[k] = v;
      return e;
    }
    D.css = c;
  ...
  D.isDomReady = function () {
    var s = doc.readyState;
    return ((s === 'complete') || (s === 'interactive'));
  };

  D.whenDomReady = function (cb) {
    /* Add a short delay always, so I can register handlers immediately
      even if I also D.setWindow() very soon. */
    setTimeout(function checkReady() {
      if (D.isDomReady()) { return cb(); }
